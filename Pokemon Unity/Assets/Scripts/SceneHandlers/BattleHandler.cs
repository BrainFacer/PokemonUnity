...
public class BattleHandler : MonoBehaviour
{
    ...
    {	
		Vector2 shadowSize = new Vector2 (4, 4);
		Vector2 normalSize = pokemon.sprite.bounds.size * 17;
	...
	private IEnumerator opponentScale(Pokemon p, Image i){
		int id = p.getID ();
		try {
			Vector3 offset = new Vector3 (PokemonDatabase.getOffsetX (id), PokemonDatabase.getOffsetY (id), 0);
			Debug.Log(i.gameObject.GetComponent<RectTransform>().position);
			i.gameObject.transform.position = i.gameObject.transform.position + offset;
...
//		Debug.Log(pokemon[3].getName()+": HP: "+pokemon[3].getHP()+"ATK: "+pokemon[3].getATK()+"DEF: "+pokemon[3].getDEF()+"SPA: "+pokemon[3].getSPA()+"SPD: "+pokemon[3].getSPD()+"SPE:"+pokemon[3].getSPE());
        ...
        if (trainerBattle)
        {
			...
			shadow.gameObject.GetComponent<RectTransform> ().sizeDelta = new Vector2(opponent1.sprite.bounds.size.y * 25, opponent1.sprite.bounds.size.y * 10);
            yield return StartCoroutine(releasePokemon(opponent1));
			yield return StartCoroutine (opponentScale(pokemon[3], opponent1));
            PlayCry(pokemon[3]);
            yield return new WaitForSeconds(0.3f);
